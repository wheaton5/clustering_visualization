...
class PlayQuicksort(Scene):
    ...
    def construct(self):
        def create_text(text_object):
            return Create(text_object, run_time = 0.075*len(text_object[0]))
        ...
        new_number_objects = []
        animations = []
        animations.append(Indicate(text3, run_time=2))
        ...
        square_objects = []
        animations = []
        for (index, data_point) in enumerate(first_data):
            number = Text(str(data_point))
            number.move_to(get_x_shift(index, n, side_length))
            number.shift(array_y_location)
            number.scale(text_scale)
            square = Square(side_length= side_length)
            square.move_to(get_x_shift(index, n, side_length))
            square.shift(array_y_location)
            
            animations.append(FadeIn(number))
            animations.append(FadeIn(square))
            square_objects.append(square)
            number_objects.append(number)
        newsquare = Square(side_length=side_length).move_to(array_y_location).set_color("#FF0000").shift(get_x_shift(2, n, side_length))
        animations.append(FadeIn(newsquare))
        self.play(animations)
        self.wait()
        animations = [FadeOut(text), FadeOut(text55)]
        self.play(animations)
        textpivot1 = Tex("How to partition efficiently")
        textpivot2 = Tex(r"{15cm}For this, we need a temporary variable to avoid overwriting data when swapping values", tex_environment="minipage")
        textpivot3 = Tex("1. First move the pivot to index 0.")
        textpivot4 = Tex("2. Set up two pointers, low and high at indices 1 and n-1 (0 indexed)")
        textpivot5 = Tex("3. While low is less than the pivot increment the pointer")
        textpivot6 = Tex("4. While high is greater than the pivot, decrement the pointer")
        textpivot7 = Tex("5. Swap values if low is left of high")
        textpivot8 = Tex("Continue 3-5 until the low index is $>=$ high index")
        textpivot9 = Tex("Finally, swap the pivot with the value at the high pointer")
        textpivotgroup = VGroup(textpivot1, textpivot2, textpivot3, textpivot4, textpivot5, textpivot6, textpivot7, textpivot8, textpivot9).arrange(DOWN, buff=0.2).scale(0.65).shift(DOWN*2)
        self.play(create_text(textpivot1))
        self.wait()
        self.play(create_text(textpivot2))
        tmpsquare = Square(side_length).shift(array_y_location).shift(DOWN*1.35)
        tmptext = Tex("temp").align_to(tmpsquare, DOWN).shift(DOWN*0.35).scale(0.5)
        self.play(FadeIn(tmpsquare), FadeIn(tmptext))
        self.wait()
        self.play(create_text(textpivot3))
        def swap(array, i, j):
            tmp = array[i]
            array[i] = array[j]
            array[j] = tmp
        self.wait()
        line = Line(square_objects[0].get_center(), tmpsquare.get_center())
        self.play(MoveAlongPath(number_objects[0], line), run_time = 2)
        self.wait()
        line = Line(square_objects[2].get_center(), square_objects[0].get_center())
        self.play(MoveAlongPath(number_objects[2], line), MoveAlongPath(newsquare, line), run_time = 2)
        self.wait()
        line = Line(tmpsquare.get_center(), square_objects[2].get_center())
        self.play(MoveAlongPath(number_objects[0], line), run_time=2)
        swap(number_objects, 0, 2)
        swap(first_data, 0, 2)

        comparison_runtime = 1
        move_runtime = 1
        low = Tex("low").move_to(square_objects[1].get_center()).scale(0.6).shift(DOWN*0.65)
        high = Tex("high").move_to(square_objects[15].get_center()).scale(0.6).shift(DOWN*0.65)
        self.play(create_text(textpivot4))
        self.wait()
        self.play(FadeIn(low), FadeIn(high))
        self.wait()
        self.play(create_text(textpivot5))    
        nope = Tex(str(first_data[1]) +" $<$ " + str(first_data[0]) + "? false").move_to(square_objects[1]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        self.wait()
        self.play(create_text(textpivot6))
        self.wait()
        nope = Tex(str(first_data[-1])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[-1]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line), FadeOut(nope), run_time=move_runtime)
        self.wait() 
        
        nope = Tex(str(first_data[14])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[14]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[13])+" $>$ " + str(first_data[0]) + "? false").move_to(square_objects[13]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(create_text(textpivot7))
        self.wait()
        line = Line(square_objects[1].get_center(), tmpsquare.get_center())
        self.play(MoveAlongPath(number_objects[1], line), run_time=move_runtime)
        line = Line(square_objects[13].get_center(), square_objects[1].get_center())
        self.play(MoveAlongPath(number_objects[13], line), run_time=move_runtime)
        line = Line(tmpsquare.get_center(), square_objects[13].get_center())
        self.play(MoveAlongPath(number_objects[1], line), FadeOut(nope), run_time=move_runtime)
        swap(number_objects, 1, 13)
        swap(first_data, 1, 13)
        self.wait()
        self.play(create_text(textpivot8))
        self.wait()
        self.play(Indicate(textpivot5), run_time=2)
        nope = Tex(str(first_data[1])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[1]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        
        nope = Tex(str(first_data[2])+" $<$ " + str(first_data[0]) + "? false").move_to(square_objects[2]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        self.play(Indicate(textpivot6), run_time=move_runtime)
        nope = Tex(str(first_data[13])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[13]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[12])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[12]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[11])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[11]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[10])+" $>$ " + str(first_data[0]) + "? false").move_to(square_objects[10]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        self.wait()
        
        self.play(Indicate(textpivot7), run_time=2)
        self.wait()
        line = Line(square_objects[2].get_center(), tmpsquare.get_center())
        self.play(MoveAlongPath(number_objects[2], line), run_time=move_runtime)
        self.wait()
        line = Line(square_objects[10].get_center(), square_objects[2].get_center())
        self.play(MoveAlongPath(number_objects[10], line), run_time=move_runtime)
        self.wait()
        line = Line(tmpsquare.get_center(), square_objects[10].get_center())
        self.play(MoveAlongPath(number_objects[2], line), run_time=move_runtime)
        self.wait()
        swap(number_objects, 2, 10)
        swap(first_data, 2, 10)
        self.play(Indicate(textpivot5), run_time=2)
        self.wait()
        nope = Tex(str(first_data[2])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[2]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[3])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[3]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line),FadeOut(nope), run_time=move_runtime)
        nope = Tex(str(first_data[4])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[4]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line), FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[5])+" $<$ " + str(first_data[0]) + "? false").move_to(square_objects[5]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait() 
        self.play(FadeOut(nope))
        self.wait()
        self.play(Indicate(textpivot6), run_time=2)
        self.wait()
        nope = Tex(str(first_data[10])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[10]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line),FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[9])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[9]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line),FadeOut(nope), run_time=move_runtime)
        nope = Tex(str(first_data[8])+" $>$ " + str(first_data[0]) + "? false").move_to(square_objects[8]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        self.wait()
        self.play(Indicate(textpivot7), run_time=2)
        line = Line(number_objects[5].get_center(), tmpsquare.get_center())
        self.play(MoveAlongPath(number_objects[5], line), run_time=move_runtime)
        self.wait()
        line = Line(square_objects[8].get_center(), square_objects[5].get_center())
        self.play(MoveAlongPath(number_objects[8], line), run_time=move_runtime)
        self.wait()
        line = Line(tmpsquare.get_center(), square_objects[8].get_center())
        self.play(MoveAlongPath(number_objects[5], line), run_time=move_runtime)
        self.wait()
        swap(number_objects, 5, 8)
        swap(first_data, 5, 8)
        

        self.wait()
        nope = Tex(str(first_data[5])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[5]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line),FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[6])+" $<$ " + str(first_data[0]) + "? true").move_to(square_objects[6]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(low.get_center(), low.get_center()+RIGHT*side_length)
        self.play(MoveAlongPath(low, line),FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[7])+" $<$ " + str(first_data[0]) + "? false").move_to(square_objects[7]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        
        self.play(Indicate(textpivot6), run_time=2)
        nope = Tex(str(first_data[8])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[8]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line),FadeOut(nope), run_time=move_runtime)
        self.wait()
        nope = Tex(str(first_data[7])+" $>$ " + str(first_data[0]) + "? true").move_to(square_objects[7]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        line = Line(high.get_center(), high.get_center()+LEFT*side_length)
        self.play(MoveAlongPath(high, line),FadeOut(nope), run_time=move_runtime)
        nope = Tex(str(first_data[6])+" $>$ " + str(first_data[0]) + "? false").move_to(square_objects[6]).shift(UP*0.6).scale(0.5)
        self.play(FadeIn(nope), run_time=comparison_runtime)
        self.wait()
        self.play(FadeOut(nope))
        self.play(create_text(textpivot9))
        self.wait()
        self.add(newsquare)
        line = Line(square_objects[0].get_center(), tmpsquare.get_center())
        self.play(MoveAlongPath(number_objects[0], line), MoveAlongPath(newsquare, line), run_time=2)
        self.wait()
        line = Line(square_objects[6].get_center(), square_objects[0].get_center())
        self.play(MoveAlongPath(number_objects[6], line), run_time=move_runtime)
        self.wait()
        line = Line(tmpsquare.get_center(), square_objects[6].get_center())
        self.play(MoveAlongPath(number_objects[0], line), MoveAlongPath(newsquare, line), run_time=2)
        self.wait(3)
